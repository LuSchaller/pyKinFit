...
struct setbestcombi(vector<vector<double>> inputdata, vector<vector<double>> gendata, double dRLimit) {
...
*/	
      //check if btagged jets are close enough
	if (DeltaR(*B1, *B2) <= 2) {
	    continue;
	}
...
*/	
 ...
 vector<TLorentzVector *> genJets;
    ...
    bestSelection.combinationType.clear();
    if (option == "ambiguous") {
